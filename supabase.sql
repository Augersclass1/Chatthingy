...
create policy "users can create own posts" on public.posts for insert to authenticated with check (author_id=auth.uid());
...
create policy "signed in users can create conversations" on public.conversations for insert to authenticated with check (true);
drop policy if exists "members can update group conversations" on public.conversations;
create policy "members can update group conversations" on public.conversations for update to authenticated using (public.is_conversation_member(id)) with check (public.is_conversation_member(id));

-- Members can see only membership records belonging to conversations they can access.
drop policy if exists "members can read memberships" on public.conversation_members;
create policy "members can read memberships" on public.conversation_members for select to authenticated using (public.is_conversation_member(conversation_id));
drop policy if exists "users can join conversations" on public.conversation_members;
create policy "users can join conversations" on public.conversation_members for insert to authenticated with check (user_id=auth.uid() or public.is_conversation_member(conversation_id));
drop policy if exists "users can leave conversations" on public.conversation_members;
create policy "users can leave conversations" on public.conversation_members for delete to authenticated using (user_id=auth.uid() or public.is_conversation_member(conversation_id));

-- Messages are visible only to conversation members.
...
create policy "members can send messages" on public.messages for insert to authenticated with check (user_id=auth.uid() and public.is_conversation_member(conversation_id));
...
-- Helpful grants for the browser client.
